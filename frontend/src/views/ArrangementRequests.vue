<template>
  <div>
    ...
    <div v-if="hasPendingRequests" class="text-center mb-4">
      ...
      <button
        @click="showRejectModalForAll"
        class="bg-red-500 text-white rounded px-4 py-2 hover:bg-red-600 ml-2"
      >
        Reject All
      </button>
    </div>

    <p v-if="noPendingRequestsMessage" class="text-center text-red-500">
    ...
    <table
      class="min-w-full border-collapse border border-gray-300 mx-auto"
      v-if="arrangementRequests.length > 0"
    >
      <thead>
        <tr>
        ...
        <template v-for="(groupRequests, groupId) in groupedRequests" :key="groupId">
          ...
          <tr>
            ...
            <td class="border border-gray-300 p-2" colspan="4">
              <button class="text-blue-600 hover:underline" @click="toggleGroup(groupId)">
    ...
    >
      <div class="bg-white p-6 rounded shadow-lg w-1/2">
        ...
        <div class="flex justify-end">
          <button
            @click="rejectGroupRequests(requestToRejectGroupId)"
            class="bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600"
          >
            Reject
          </button>
  ...
  computed: {
    ...
    isAllSelected() {
      return (
        ...
        this.arrangementRequests.filter((request) => request.status === 'Pending').length
      )
    }
  },
  created() {
  ...
  methods: {
    ...
    async fetchArrangementRequests() {
      ...
      try {
        ...
        console.error(
          'Error fetching arrangement requests:',
